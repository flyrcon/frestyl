...
defmodule FrestylWeb.PortfolioLive.PortfolioEditor do
  ...
  def mount(%{"id" => portfolio_id} = params, _session, socket) do
    ...
    case load_portfolio_with_account_and_blocks(portfolio_id, user) do
      {:ok, portfolio, account, content_blocks} ->
        ...
        # ADD these to your existing mount assigns:
        |> assign(:media_library_section_id, nil)
        |> assign(:media_library_section, nil)
  ...
  defp should_use_dynamic_card_layout?(_portfolio) do
    # DISABLED: Force all portfolios to use traditional editor
    false
  end
  ...
  def handle_event("toggle_add_section_dropdown", _params, socket) do
    IO.puts("🔥 PORTFOLIO EDITOR: toggle_add_section_dropdown")
  ...
  def handle_event("add_section", %{"type" => section_type}, socket) do
    ...
    case create_new_section(portfolio_id, section_type) do
      {:ok, new_section} ->
        ...
        |> put_flash(:info, "#{humanize_section_type(section_type)} section added")

        # Broadcast if live preview is enabled
  ...
  # ============================================================================
  # MISSING SECTION BUTTON EVENT HANDLERS
  # ============================================================================

  @impl true
  def handle_event("toggle_section_visibility", %{"section_id" => section_id}, socket) do
    IO.puts("🔥 PORTFOLIO EDITOR: toggle_section_visibility - #{section_id}")
    ...
    if section do
      case Portfolios.update_section(section, %{visible: !section.visible}) do
        {:ok, updated_section} ->
          ...
          socket = if socket.assigns[:editing_section] && socket.assigns.editing_section.id == section_id_int do
  ...
  @impl true
  def handle_event("show_section_media_library", %{"section_id" => section_id}, socket) do
    IO.puts("🔥 PORTFOLIO EDITOR: show_section_media_library - #{section_id}")
    section_id_int = String.to_integer(section_id)
    section = Enum.find(socket.assigns.sections, &(&1.id == section_id_int))
    ...
    if section do
      ...
      |> assign(:media_library_section, section)}
    else
      {:noreply, put_flash(socket, :error, "Section not found")}
    end
  end
  ...
  def handle_event("close_media_library", _params, socket) do
    IO.puts("🔥 PORTFOLIO EDITOR: close_media_library")
    {:noreply, socket
    |> assign(:show_media_library, false)
    |> assign(:media_library_section_id, nil)
    |> assign(:media_library_section, nil)}
  ...
  # ============================================================================
  # MISSING VIDEO MODAL EVENT HANDLERS
  # ============================================================================

  @impl true
  def handle_event("show_video_intro_modal", _params, socket) do
    IO.puts("🔥 PORTFOLIO EDITOR: show_video_intro_modal")
    {:noreply, assign(socket, :show_video_intro_modal, true)}
  end

  @impl true
  def handle_event("hide_video_intro_modal", _params, socket) do
    IO.puts("🔥 PORTFOLIO EDITOR: hide_video_intro_modal")
    {:noreply, assign(socket, :show_video_intro_modal, false)}
  end

  @impl true
  def handle_event("cancel_video_recording", _params, socket) do
    IO.puts("🔥 PORTFOLIO EDITOR: cancel_video_recording")
    {:noreply, assign(socket, :show_video_intro_modal, false)}
  end
  ...
  # Add these BEFORE any catch-all handlers:
...
defp get_portfolio_views(portfolio_id) do
  ...
  def handle_event(event_name, params, socket) do
    IO.puts("🔧 UNHANDLED EVENT: #{event_name} - #{inspect(params)}")
    {:noreply, socket}
  end
